...
import { Users as UsersIcon, Shield, Search, MoreVertical, Pencil, Trash2, Ban, Check, ChevronLeft, ChevronRight } from 'lucide-react';
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm text-slate-600 dark:text-slate-300">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                            {loading ? (
                                <tr><td colSpan="5" className="px-6 py-12 text-center"><div className="flex items-center justify-center gap-2 text-slate-400"><UsersIcon size={20} className="animate-spin" />Loading users...</div></td></tr>
                                ...
                                paginatedUsers.map((user, idx) => (
                                    <tr key={user.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-tight border ${user.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700 border-emerald-100 dark:bg-emerald-900/30 dark:text-emerald-300 dark:border-emerald-800' : 'bg-amber-50 text-amber-700 border-amber-100 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-800'}`}>
                                                {user.status}
                                            </span>
                                        </td>
