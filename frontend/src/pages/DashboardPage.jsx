...
const DashboardPage = () => {
  ...
  const calculateTotals = useCallback((eventList) => {
    ...
    eventList.forEach(event => {
      // FIX: Use 'contacts' relationship and correct pledge fields to avoid "0" display
      ...
      // Count vendor issues for host (Quotes awaiting review)
      if (event.vendors) {
  ...
  const StatCard = ({ label, value, icon: Icon, color, sub, isCurrency }) => (
  ...
  return (
    <div className="max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <button onClick={() => navigate('/vendor-catalog')} className="flex items-center gap-3 p-4 bg-brand-50 dark:bg-brand-900/20 text-brand-700 dark:text-brand-300 rounded-xl hover:bg-brand-100 transition-colors font-bold text-sm text-left group"><Briefcase size={20} className="group-hover:scale-110 transition-transform" /> Browse Vendors</button>
        ...
        {/* 4. COMMITTEE VIEWS */}
        {(user?.role === 'TREASURER' || user?.role === 'SECRETARY' || user?.role === 'COORDINATOR' || user?.role === 'GATE_OFFICER') && (
           <div className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                {loading || !stats ? <SkeletonCard count={4} /> : (
                  <>
                    <StatCard label="Total Guests" value={stats.totalGuests} icon={Users} color="purple" />
                    <StatCard label="Total Pledged" value={formatCurrency(stats.totalPledged)} icon={TrendingUp} color="blue" isCurrency />
                    <StatCard label="Collected" value={formatCurrency(stats.totalCollected)} icon={Wallet} color="emerald" isCurrency />
                    <StatCard label="Events" value={myEvents.length} icon={Calendar} color="indigo" />
                  </>
                )}
              </div>
              <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {loading ? (<tr><td colSpan="4" className="px-6 py-12 text-center text-slate-400">Loading events...</td></tr>) : myEvents.length === 0 ? (<tr><td colSpan="4" className="px-6 py-12 text-center text-slate-400">No assigned events.</td></tr>) : myEvents.map(event => (
                        <tr key={event.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                          <td className="px-6 py-4">
                            <p className="font-bold text-slate-900 dark:text-white">{event.event_name}</p>
                            <p className="text-xs text-slate-500">{formatDate(event.event_date)}</p>
                          </td>
                          <td className="px-6 py-4 text-center font-bold text-slate-900 dark:text-white">{event.contacts?.length || 0}</td>
                          <td className="px-6 py-4 text-center font-bold text-emerald-600">{formatCurrency(calculateTotals([event]).totalCollected)}</td>
                          <td className="px-6 py-4 text-right">
